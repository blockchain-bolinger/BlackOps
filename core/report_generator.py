...
class ReportGenerator:
    ...
    def generate_pentest_report(self, data: Dict[str, Any], format: str = "pdf") -> str:
        ...
        return self._write_report(report, format=format, subdir="pentest")

    def generate_vulnerability_report(self, scan_data: Dict[str, Any]) -> str:
        report_id = f"vuln_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
        report = self._build_schema("vulnerability", scan_data, report_id)
        return self._write_report(report, format="json", subdir="scans")
